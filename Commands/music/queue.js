...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!queue) return interaction.editReply({ content: await Translate(`No music currently playing <${interaction.member}>... try again ? <❌>`) });
        if (!queue.tracks.toArray()[0]) return interaction.editReply({ content: await Translate(`No music in the queue after the current one <${interaction.member}>... try again ? <❌>`) });
        ...
        const embed = new EmbedBuilder()
            ...
            .setAuthor({ name: await Translate(`Server queue - <${interaction.guild.name}> <${methods[queue.repeatMode]}>`), iconURL: interaction.client.user.displayAvatarURL({ size: 1024, dynamic: true }) })
