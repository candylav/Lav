...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: await Translate(`From the server <${interaction.guild.name}>`), iconURL: interaction.guild.iconURL({ dynamic: false }) });

        interaction.member.send({ embeds: [embed] })
            .then(async () => {
                return interaction.editReply({ content: await Translate(`I have sent you the music in private messages <✅>`) });
            })
            .catch(async () => {
                return interaction.editReply({ content: await Translate(`Unable to send you a private message... try again ? <❌>`) });
            });
    }
};
